...
export function FilterSheet({
  ...
  return (
    <div className="absolute inset-0 z-[80]" onClick={onClose}>
      ...
      >
        <div
          className="w-10 h-1 rounded-full mx-auto mb-5 flex-shrink-0"
        ...
        <div className="flex flex-1 overflow-hidden min-h-0">
          {/* Left: category menu */}
          <div
            className="flex-shrink-0 overflow-y-auto hide-scrollbar"
            style={{ width: 120, background: "var(--premium-surface-soft)" }}
          >
            {categories.map((cat) => {
              ...
              <div>
                ...
                <div className="flex flex-col gap-0.5">
                  {currentCategory.options.map((opt) => {
                    const isSelected =
                      localSelected[currentCategory.id]?.has(opt.id) ?? false;
                    return (
                      <button
                        key={opt.id}
