...
export function SpaceEditor({
  ...
  const toggleCustomTemplate = (id: string) =>
    setEnabledCustomTemplates((prev) => {
      const next = new Set(prev);
      next.has(id) ? next.delete(id) : next.add(id);
      return next;
    });
  const handleSave = () => {
    const name =
      spaceName.trim() ||
      SPACE_TYPES.find((t) => t.id === selectedType)?.name ||
      "新资料箱";
    onSave({ name, emoji: selectedEmoji, type: selectedType });
  };
      ...
      <div className="relative z-10 flex items-center justify-between px-4 pt-2 pb-3">
        ...
        >
          <span
            ...
            style={{
              fontSize: 17,
              color: spaceName.trim() ? "#2563FF" : "var(--premium-chevron)",
            }}
          >
            保存
          </span>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto hide-scrollbar pb-10">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {type.pro && (
                        <div className="flex items-center gap-1">
                          <Lock size={10} className="text-[#7C5CFF]" />
                          <span
        ...
        {isProType && (
          <div className="px-4 mb-5">
            ...
            >
              <Lock size={14} className="text-[#7C5CFF] flex-shrink-0 mt-0.5" />
              <div>
                <p
                  className="font-semibold text-[#7C5CFF]"
                  style={{ fontSize: 13 }}
                >
                  需要 Pro 订阅
                </p>
                <p
                  className="mt-0.5"
                  style={{ fontSize: 12, color: "#A694FF" }}
                >
                  自定义资料箱可自由设置对象类型和字段
                </p>
              </div>
        ...
        {currentTemplates.length > 0 && (
          <div className="px-4 mb-5">
            ...
            >
              {currentTemplates.map((tmpl, i) => {
                const isOn = enabledTemplates.has(tmpl.id);
                return (
                  <button
                    key={tmpl.id}
        ...
        <div className="px-4 mb-5">
          ...
          >
            {CUSTOM_TEMPLATES_FROM_OTHER_SPACES.map((tmpl, i) => {
              const isOn = enabledCustomTemplates.has(tmpl.id);
              return (
                <button
                  key={tmpl.id}
                  className="ios-tap w-full flex items-center px-4 py-3 text-left"
                  style={{
                    borderBottom:
                      i < CUSTOM_TEMPLATES_FROM_OTHER_SPACES.length - 1
                        ? "0.5px solid var(--premium-row-border)"
                        : "none",
                  }}
                  onClick={() => toggleCustomTemplate(tmpl.id)}
                  aria-label={tmpl.name}
                >
                  <div
                    className="flex items-center justify-center flex-shrink-0 mr-3"
          ...
          <button
            className="ios-tap w-full flex items-center justify-center gap-2 rounded-xl py-3.5 mt-2"
            style={{
