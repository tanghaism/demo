    ...
    <div className="relative flex flex-col h-full overflow-hidden" style={{ paddingTop: 54 }}>
        ...
        <div className="px-4 mt-3">
          <PremiumCard className="rounded-[18px]">
            ...
            <button
              ...
              style={{ height: 56, borderBottom: "0.5px solid var(--premium-row-border)" }}
              onClick={() => setShowTypeSheet(true)}
            >
              <span className="font-medium flex-shrink-0 mr-4" style={{ fontSize: 14, width: 56, color: "var(--premium-text-subtle)" }}>类型</span>
              <div className="flex items-center gap-2 flex-1 justify-end">
                {activeType && (
                  <>
                    <span style={{ fontSize: 18 }}>{activeType.emoji}</span>
                    <span style={{ fontSize: 15, color: "var(--premium-text)" }}>{activeType.label}</span>
                  </>
                )}
                <ChevronRight size={16} strokeWidth={2} className="flex-shrink-0" style={{ color: "var(--premium-chevron)" }} />
              </div>
            </button>
            ...
            >
              ...
              <div className="flex items-center gap-2 flex-1 justify-end">
                <div className="flex items-center justify-center" style={{ width: 36, height: 36, borderRadius: 10, background: "var(--premium-icon-blue-bg)" }}>
      ...
      {showTypeSheet && (
        <div className="absolute inset-0 z-50" onClick={() => setShowTypeSheet(false)}>
          ...
          <div className="absolute bottom-0 left-0 right-0 rounded-t-3xl pt-2 pb-8" style={{ background: "var(--premium-surface)", boxShadow: "var(--premium-action-shadow)", maxHeight: "80%" }} onClick={(e) => e.stopPropagation()}>
            <div className="w-10 h-1 rounded-full mx-auto mb-5" style={{ background: "var(--premium-row-border)" }} />
            <p className="font-bold px-5 mb-3" style={{ fontSize: 18, color: "var(--premium-text)" }}>选择类型</p>
              ...
              {OBJECT_TYPES.map((type) => {
                ...
                return (
                  <button
                    ...
                    style={{ background: isActive ? "var(--premium-surface-selected)" : "transparent" }}
                    onClick={() => { setSelectedType(type.id); setSelectedEmoji(type.emoji); setShowTypeSheet(false) }}
                  >
                    <div className="flex items-center justify-center flex-shrink-0 mr-3" style={{ width: 40, height: 40, borderRadius: 10, background: "var(--premium-icon-blue-bg)" }}>
                      <span style={{ fontSize: 22 }}>{type.emoji}</span>
                    </div>
                    <div className="flex-1 text-left">
                      <p className="font-medium" style={{ fontSize: 15, color: "var(--premium-text)" }}>{type.label}</p>
                      <p style={{ fontSize: 12, color: "var(--premium-text-subtle)" }}>{type.desc}</p>
                    </div>
      ...
      {showEmojiSheet && (
        <div className="absolute inset-0 z-50" onClick={() => setShowEmojiSheet(false)}>
          ...
          <div className="absolute bottom-0 left-0 right-0 rounded-t-3xl pt-2 pb-8" style={{ background: "var(--premium-surface)", boxShadow: "var(--premium-action-shadow)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <p className="font-bold px-5 mb-3" style={{ fontSize: 18, color: "var(--premium-text)" }}>选择图标</p>
              ...
              {EMOJI_OPTIONS.map((emoji) => (
                <button
                  ...
                  style={{
                    ...
                    aspectRatio: "1",
                    fontSize: 28,
                    background: selectedEmoji === emoji ? "var(--premium-surface-selected)" : "var(--premium-surface-soft)",
