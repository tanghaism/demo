    ...
    <div className="absolute inset-0 z-[80] flex items-end" onClick={onClose}>
      ...
      <div className="relative w-full rounded-t-3xl pt-2 pb-8 premium-sheet-panel" style={{ background: "var(--premium-surface)", boxShadow: "var(--premium-action-shadow)" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="flex flex-col items-center text-center px-5 mb-5">
          ...
          <p className="font-bold" style={{ fontSize: 18, color: "var(--premium-text)" }}>资料包已生成</p>
          <p className="mt-2 leading-snug" style={{ fontSize: 14, color: "var(--premium-text-subtle)" }}>
    ...
    <div className="relative flex flex-col h-full overflow-hidden" style={{ paddingTop: 54 }}>
      ...
      <div className="relative z-10 flex-1 overflow-y-auto hide-scrollbar pb-28">
        ...
        <div className="px-4 mt-4 mb-3">
          <p className="font-medium px-1 mb-1.5" style={{ fontSize: 13, color: "var(--premium-text-subtle)" }}>导出范围</p>
          <PremiumCard className="rounded-[18px]">
            <div className="flex items-center px-4 py-4">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center mr-3 flex-shrink-0" style={{ background: "var(--premium-icon-blue-bg)" }}>
        ...
        <div className="px-4 mb-3">
          <p className="font-medium px-1 mb-1.5" style={{ fontSize: 13, color: "var(--premium-text-subtle)" }}>ZIP 包含内容</p>
            ...
            <div className="grid grid-cols-3 gap-3 mb-3">
              {[
                ...
                <div key={stat.label} className="text-center">
                  ...
                  <p style={{ fontSize: 12, color: "var(--premium-text-muted)" }}>{stat.label}</p>
                </div>
              ))}
            </div>
            <div className="pt-3 flex flex-col gap-2" style={{ borderTop: "0.5px solid var(--premium-row-border)" }}>
              <div className="flex items-center gap-2">
                <FileText size={14} className="text-[#FF3B30] flex-shrink-0" />
                <span style={{ fontSize: 12, color: "var(--premium-text-muted)" }}>摘要 PDF（记录字段 + 事件时间线）</span>
              </div>
              <div className="flex items-center gap-2">
                <Archive size={14} className="text-[#FF9500] flex-shrink-0" />
                <span style={{ fontSize: 12, color: "var(--premium-text-muted)" }}>原始附件（图片、PDF、各类文件）</span>
              </div>
            </div>
            <div className="pt-3" style={{ borderTop: "0.5px solid var(--premium-row-border)" }}>
              <p className="text-center" style={{ fontSize: 13, color: "var(--premium-text-subtle)" }}>预计大小：约 12 MB</p>
            </div>
          </PremiumCard>
        </div>
        ...
        {/* Disclaimer */}
        <div className="px-4">
          <div className="flex items-start gap-3 px-4 py-3 rounded-xl" style={{ background: "var(--premium-warning-bg)" }}>
      ...
      {/* Bottom CTA */}
      <div className="absolute bottom-0 left-0 right-0 z-20 px-4 pb-7 pt-3" style={{ background: "var(--premium-surface)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderTop: "0.5px solid var(--premium-row-border)" }}>
        {tier === "free" ? (
          <div className="flex flex-col gap-2">
            <button className="ios-tap premium-press w-full rounded-2xl font-semibold" style={{ height: 50, fontSize: 17, background: "linear-gradient(135deg, #5B7CFF 0%, #7B61FF 100%)", color: "#FFFFFF" }} onClick={() => setShowFreePreview(true)}>预览资料包</button>
            <button className="ios-tap w-full rounded-2xl font-semibold" style={{ height: 50, fontSize: 17, background: "var(--premium-warning-bg)", color: "var(--premium-warning-text)" }} onClick={() => onNavigate("pro-upgrade")}>升级 Pro 解锁导出</button>
          </div>
        ) : (
          <button className="ios-tap premium-press w-full flex items-center justify-center gap-2 rounded-2xl font-semibold text-white" style={{ height: 50, fontSize: 17, background: "linear-gradient(135deg, #5B7CFF 0%, #7B61FF 100%)" }} onClick={() => setShowGenerated(true)}>
        ...
        <div className="absolute inset-0 z-[80] flex items-end" onClick={() => setShowFreePreview(false)}>
          ...
          <div className="relative w-full rounded-t-3xl pt-2 pb-8 premium-sheet-panel" style={{ background: "var(--premium-surface)", boxShadow: "var(--premium-action-shadow)" }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col items-center text-center px-5 mb-5">
              ...
              <p className="font-bold" style={{ fontSize: 18, color: "var(--premium-text)" }}>预览资料包</p>
              <p className="mt-2 leading-snug" style={{ fontSize: 14, color: "var(--premium-text-subtle)" }}>免费版可预览 ZIP 文件结构，正式导出需要升级 Pro。</p>
              <div className="w-full mt-4 rounded-xl p-4 text-left" style={{ background: "var(--premium-surface-soft)" }}>
                <p className="font-medium mb-2" style={{ fontSize: 13, color: "var(--premium-text)" }}>📁 {scopeInfo.label}.zip</p>
                <div className="flex flex-col gap-1">
                  {["├── summary.pdf", "├── macbook_invoice.pdf", "├── IMG_60512.jpg", "├── IMG_60511.MOV", "└── warranty_card.pdf"].map((f) => (
                    <p key={f} className="font-mono" style={{ fontSize: 12, paddingLeft: 4, color: "var(--premium-text-subtle)" }}>{f}</p>
                  ))}
                </div>
              </div>
            </div>
